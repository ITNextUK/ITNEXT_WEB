...
const Research: React.FC = () => {
  ...
  const getIcon = (idx: string) => {
    switch(idx) {
      case 'poc': return <Beaker size={32} />;
      case 'framework': return <Layers size={32} />;
      case 'innovation': return <TrendingUp size={32} />;
      default: return <Beaker size={32} />;
    }
  };

  const getDecorative = (idx: string) => {
    switch(idx) {
      case 'poc': return <TestTube size={240} />;
      case 'framework': return <Search size={240} />;
      case 'innovation': return <Lightbulb size={240} />;
      default: return <TestTube size={240} />;
    }
  };
      ...
      <section className="py-24 px-6 bg-zinc-50/30">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {research.pillars.map((pillar, index) => {
              const associatedProducts = content.products?.filter(p => p.researchCategoryId === pillar.id) || [];
              return (
                <div key={pillar.id} className="group relative overflow-hidden bg-white rounded-[3rem] border border-zinc-200 p-12 flex flex-col justify-between hover:border-brand-accent transition-all duration-700 shadow-sm hover:shadow-2xl hover:shadow-brand-accent/10 h-full min-h-[600px]">
                    ...
                    <div className="space-y-8">
                      ...
                      {associatedProducts.length > 0 && (
                        <div className="pt-8 border-t border-zinc-100">
                          <h4 className="text-[9px] font-black uppercase tracking-[0.4em] text-brand-accent mb-4">Matured Assets</h4>
                          <div className="grid grid-cols-1 gap-3">
                            {associatedProducts.map(product => (
                              <a key={product.id} href={product.url} target="_blank" rel="noopener noreferrer" className="group/prod flex items-center justify-between p-5 rounded-3xl bg-brand-dark text-white border border-brand-dark hover:border-brand-accent transition-all duration-500 shadow-xl shadow-brand-dark/10">
                                <div>
                                  <h5 className="text-[11px] font-black uppercase tracking-widest text-brand-accent mb-1">{product.name}</h5>
                                  <p className="text-[9px] text-zinc-400 font-medium uppercase tracking-widest">{product.targetMarket}</p>
                                </div>
                                <ArrowUpRight size={16} className="text-zinc-500 group-hover/prod:text-white group-hover/prod:translate-x-1 group-hover/prod:-translate-y-1 transition-all" />
                              </a>
                            ))}
                          </div>
                        </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24">
            <div>
              <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-brand-accent mb-6">{research.ethics.subtitle || 'Research Ethics'}</h2>
              <h3 className="text-4xl font-black text-brand-dark mb-10 tracking-tight uppercase">{research.ethics.title}</h3>
              <p className="text-lg text-zinc-500 leading-relaxed mb-12 font-medium">
                {research.ethics.description}
              </p>
              <div className="space-y-8">
                {research.ethicsItems.map((item, i) => (
